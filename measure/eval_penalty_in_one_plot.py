...
def plot(results):
    penalty_names = results.itervalues().next().keys()
    ...
    offset = -0.4
    for penalty_name, result in results.iteritems():
        plt.barh(ind + offset + fi, result.values(), align='edge',
                 height=height, color=colors[i], hatch=hatches[i], label=penalty_name)
        fi = fi + height
        i = i + 1
