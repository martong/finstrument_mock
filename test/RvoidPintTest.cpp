...
void fake_RvoidPint_3(int a); // user provides
// should be generated by a macro
void fake_RvoidPint_3_aux(int count, ...) {
    ::ftest::called.insert(reinterpret_cast<char *>(fake_RvoidPint_3_aux));

    EXPECT_EQ(count, 1);

    va_list args;
    va_start(args, count);
    fake_RvoidPint_3(va_arg(args, int));
    va_end(args);
}
